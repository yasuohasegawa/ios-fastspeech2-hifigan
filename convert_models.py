...
model_tag = "espnet/kan-bayashi_jsut_fastspeech2"
tts = Text2Speech.from_pretrained(model_tag=model_tag, device="cpu")

print("--- Manually loading HiFi-GAN vocoder ---")
vocoder_config_path = os.path.expanduser("~/.cache/parallel_wavegan/just_hifigan.v1/config.yml")
vocoder_ckpt_path = os.path.expanduser("~/.cache/parallel_wavegan/just_hifigan.v1/checkpoint-2500000steps.pkl")
try:
...
with open("jsut_phoneme_map.json", "w", encoding="utf-8") as f:
    json.dump(phoneme_map, f, ensure_ascii=False, indent=2)
print("✅ Phoneme map saved")

sample_phonemes = ['k', 'o', 'N', 'n', 'i', 'ch', 'i', 'w', 'a']
unk = phoneme_map.get("<unk>", 0)
sample_ids = [phoneme_map.get(p, unk) for p in sample_phonemes]
input_ids = torch.LongTensor(sample_ids).unsqueeze(0)
